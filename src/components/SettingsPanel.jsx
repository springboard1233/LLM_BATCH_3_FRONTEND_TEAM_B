...
const SettingsPanel = ({ isOpen, onClose }) => {
  ...
  const modalContent = (
    ...
    >
      <div 
        ref={modalRef}
        className="relative w-full max-w-2xl max-h-[90vh] bg-black/80 backdrop-blur-md rounded-xl border border-white/20 shadow-2xl overflow-hidden animate-modal-in"
      >
        {/* Header */}
          ...
          <div className="space-y-8">
            ...
            {(!isStorageAvailable() || getStorageInfo().error) && (
              <div className="bg-orange-500/20 border border-orange-500/30 rounded-lg p-4 mb-6">
                <h4 className="text-orange-300 font-medium mb-2">⚠️ Storage Notice</h4>
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <label className="text-white font-medium">{t('settings.preferences.notifications')}</label>
                    <p className="text-sm text-gray-400">{t('settings.preferences.notificationsDesc')}</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <label className="text-white font-medium">{t('settings.preferences.autoRefresh')}</label>
                    <p className="text-sm text-gray-400">{t('settings.preferences.autoRefreshDesc')}</p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <label className="text-white font-medium">{t('settings.preferences.compactMode')}</label>
                    <p className="text-sm text-gray-400">{t('settings.preferences.compactModeDesc')}</p>
                  </div>
                  <button
        ...
        <div className="flex items-center justify-between p-6 border-t border-white/10 bg-black/20">
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg transition-colors"
          >
            ...
            <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full border border-white/20">
              ...
              <div className="flex space-x-3">
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-gray-600 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg transition-colors"
                >
                  {t('settings.cancel')}
                </button>
                <button
                  onClick={handleReset}
                  disabled={isResetting}
                  className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg transition-colors flex items-center justify-center"
                >
                  {isResetting ? (
                    <>
